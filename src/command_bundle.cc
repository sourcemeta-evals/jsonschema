...
auto sourcemeta::jsonschema::bundle(const sourcemeta::core::Options &options)
  ...
  if (options.contains("without-id")) {
    LOG_WARNING() << "You are opting in to remove schema identifiers in "
                      "the bundled schema.\n"
                   << "The only legit use case of this advanced feature we "
                      "know of is to workaround\n"
                   << "non-compliant JSON Schema implementations such as "
                      "Visual Studio Code.\n"
                   << "Otherwise, this is not needed and may harm other use "
                      "cases. For example,\n"
                   << "you will be unable to reference the resulting schema "
                      "from other schemas\n"
