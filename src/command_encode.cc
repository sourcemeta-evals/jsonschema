...
auto sourcemeta::jsonschema::cli::encode(
  ...
  const std::filesystem::path document{options.at("")[0]};
  ...
  if (document.extension() == ".jsonl") {
    ...
    std::ofstream output_stream(safe_weakly_canonical(options.at("")[1]),
                                std::ios::binary);
    ...
    const auto entry{sourcemeta::jsonschema::cli::read_file(options.at("")[0])};
    std::ofstream output_stream(safe_weakly_canonical(options.at("")[1]),
                                std::ios::binary);
    output_stream.exceptions(std::ios_base::badbit);
    sourcemeta::jsonbinpack::Encoder encoder{output_stream};
