...
# Save the original file's modification time and content
...
if [ "$ORIGINAL_MTIME" != "$NEW_MTIME" ]; then
  echo "FAIL: File modification time changed when it should not have"
  exit 1
fi

if [ "$ORIGINAL_CONTENT" != "$NEW_CONTENT" ]; then
  echo "FAIL: File content changed when it should not have"
  echo "Original:"
...
echo "PASS: File was not modified"
