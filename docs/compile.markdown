...
The `--minify`/`-m` option prints the compiled schema template in stringified
(minified) form without pretty-printing, which can be useful for reducing file
size or for machine processing.
