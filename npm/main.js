...
function spawn(args, options) {
  return new Promise((resolve, reject) => {
    if (!Array.isArray(args)) {
      reject(new Error('The first argument must be an array of strings'));
      return;
    }
    ...
    const useInherit = spawnOptions.stdio === 'inherit';
    ...
    child.on('close', (code) => {
      resolve({
        stdout: useInherit ? null : stdout,
        stderr: useInherit ? null : stderr,
        code: code
      });
    });
  });
...
module.exports = { spawn, EXECUTABLE };
