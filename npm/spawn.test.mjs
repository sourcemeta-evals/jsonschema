...
test('spawn with json option keeps string stdout on parse failure', async () => {
  // --version doesn't output JSON, so parsing should fail gracefully
...
test('spawn without json option returns string stdout', async () => {
  ...
  try {
    ...
    assert.strictEqual(result.code, 0);
    // Without json option, stdout should be a string
    assert.strictEqual(typeof result.stdout, 'string');
  } finally {
    fs.rmSync(tmpDir, { recursive: true });
  }
