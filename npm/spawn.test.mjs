...
test('spawn with json option appends --json and parses output', async () => {
  ...
  try {
    ...
    assert.strictEqual(result.code, 0);
    assert.strictEqual(typeof result.stderr, 'object');
    assert.ok(result.stderr.locations);
    assert.ok(result.stderr.references);
  } finally {
